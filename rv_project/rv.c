...
static void *cont_worker(void *arg){
    ...
    free(fpws); w->done=1; sem_post_s(w->slot);
    return NULL;
}
...
static void encode_otc(const char *folder,int remove_original,int ignore_swap){
    ...
    if(fl.cnt > 10000000){fl_free(&fl);die("too many files");} /* guard against malloc bombs */
    ...
    if(remove_original){
        for(size_t i=0;i<fl.cnt;i++){
            char abs[MAX_PATH_LEN]; snprintf(abs,sizeof(abs),"%s/%s",root,fl.paths[i]); unlink(abs);
        }
        rmdir(root);
    }
}
...
int main(int argc,char **argv){
    printf("\n╔════════════════════════════════════════════════════╗\n");
    printf("║  RV – Quantum-Resistant Encrypted Container       ║\n");
    printf("║  COPYRIGHT MANYHOST — All Rights Reserved         ║\n");
    printf("║  Build: v5 | Crypto: ML-KEM-1024 + SHAKE-256     ║\n");
    printf("╚════════════════════════════════════════════════════╝\n\n");
    ...
    if(argc<3){
        dprintf(STDERR_FILENO,
            "usage:\n"
            "  %s [--mode rv2] encode <folder|file> [remove] [-i]\n"
            "  %s [--mode rv2] decode <name_root.otc> <name.key> [-i]\n"
            "  %s [--mode rv2] run    <file>\n"
            "\nflags:\n"
            "  -i    ignore swap failsafe (allows system to use swap memory)\n"
            "  --mode rv2  maintain rv2 command-line compatibility\n",
            argv[0],argv[0],argv[0]);
        return 1;
    }

    /* support legacy rv2 mode option */
    int mode = 0; // 0 = rv5, 1 = rv2
    ...
    if(mode == 1){
        ...
        const char *pw = argv[5];
        int backup = (argc >= 6 && !strcmp(argv[6], "backup"));
        /* FIX #1: Path traversal - use realpath() for canonicalization */
        ...
        if(!strcmp(cmd,"encode")){
            ...
            if(S_ISDIR(st.st_mode))
                rv2_encode_folder(path, pw, backup);
            else
                rv2_encode_file(path, pw, backup);
        } else if(!strcmp(cmd,"decode")){
            /* detect container vs plain file by extension */
            const char *ext = strrchr(path, '.');
            if(ext && !strcmp(ext, ".rv2"))
                rv2_decode_container(path, pw);
            else
                rv2_decode_file(path, pw);
    ...
    int argi = 1;
    if(argc < 3) {
        dprintf(STDERR_FILENO,"missing command\n"); return 1;
    }
    const char *cmd = argv[argi];
    const char *path = argv[argi+1];
    if(strstr(path,".."))_exit(1);

    /* Check for -i flag to ignore swap failsafe */
    int ignore_swap = 0;
    ...
    if(!strcmp(cmd,"encode")){
        ...
        if(stat(path,&st)!=0){perror(path);return 1;}
        if(S_ISDIR(st.st_mode)){
            encode_otc(path,(argc>=4&&!strcmp(argv[3],"remove")),ignore_swap);
        } else {
            /* Single file: one container, one file */
            ...
            /* file label should mirror decoding logic: use container base without ".cont" */
